...
(defn find-state-in-dir [direction needed-state? start-pos capture-color board capture]
...
(defn find-move-in-dir [direction color start-pos board]
  (find-state-in-dir direction empty-cell? start-pos (rival-color color) board false))
...
(defn find-covering-stone-in-dir [direction color start-pos board]
    (find-state-in-dir direction #(cell-color? color %1 %2) start-pos (rival-color color) board true))
